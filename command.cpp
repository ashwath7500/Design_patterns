...
class ComplexCommand : public Command {
 private:
  ...
  string b_;
 public:
  ComplexCommand(shared_ptr<Receiver> receiver, std::string a, std::string b) : receiver_(receiver), a_(a), b_(b) {
  }
  void Execute() const override {
...
class Invoker {
 ...
 public:
  void SetOnStart(unique_ptr<Command>&& command) {
    this->on_start_ = move(command);
  }
  void SetOnFinish(unique_ptr<Command>&& command) {
    this->on_finish_ = move(command);
  }
...
int main() {
  ...
  invoker->SetOnFinish(make_unique<ComplexCommand>(receiver, "Send email", "Save report"));
